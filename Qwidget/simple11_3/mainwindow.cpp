    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(ui->tableView,SIGNAL(doubleClicked(QModelIndex)),this,SLOT(on_tableView_doubleClicked(const QModelIndex &index)));
}
...
void MainWindow::updateRecord(int recNo)
{
    //更新一条记录
        ...
        DialogData    *dataDialog=new DialogData(this); //创建对话框
...
{//插入记录
    ...
    DialogData    *dataDialog=new DialogData(this);
    Qt::WindowFlags    flags=dataDialog->windowFlags();
    dataDialog->setWindowFlags(flags | Qt::MSWindowsFixedSizeDialogHint); //设置对话框固定大小
    dataDialog->setInserRecord(curRec); //插入记录
    int ret=dataDialog->exec();// 以模态方式显示对话框
    if (ret==QDialog::Accepted) //OK键被按下
    {
